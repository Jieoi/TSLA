#!/usr/bin/env python3
"""Standardized training pipeline for market news neural network classification.

This script reproduces the workflow from `market_news_neural_net.ipynb`.
...
DATA_PATH_CANDIDATES = [
    "daily_market_topic_sentiment_price.csv",
    os.path.join("..", "data", "process_data", "news", "daily_market_topic_sentiment_price.csv"),
]
...
MODEL_SAVE_PATH = os.path.join(OUTPUT_DIR, "market_news_price_classifier_model.keras")
PREDICTIONS_CSV = os.path.join(OUTPUT_DIR, "market_news_predictions.csv")
...
def load_dataset(path: str) -> pd.DataFrame:
    ...
    if "T_price" in df.columns and "T_minus_1_price" in df.columns:
...
def main() -> None:
    ...
    print("=" * 80)
    print("MARKET NEWS PRICE DIRECTION CLASSIFIER")
    ...
    # Prepare artifacts for Shapley value analysis
    shapley_data_path = os.path.join(OUTPUT_DIR, "market_news_shapley_data.npz")
