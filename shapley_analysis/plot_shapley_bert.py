"""
Plot Shapley Value Analysis Results for BERT Models
===================================================
Generates comparison and per-model plots using consolidated SHAP outputs.
"""
...
def load_shapley_results(model_name: str):
    """Load SHAP results for a model if they exist."""
    results_path = SHAPLEY_OUTPUT_DIR / f"shapley_{model_name}.pkl"
...
def plot_importance_comparison() -> None:
    """Plot summary comparisons across all BERT models."""
    ...
    fig.suptitle("Shapley Value Analysis: BERT Model Comparison", fontsize=16, fontweight="bold")
    ...
    plt.tight_layout()
    output_path = SHAPLEY_OUTPUT_DIR / "shapley_comparison.png"
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    print(f"[OK] Saved comparison plot: {output_path}")
    plt.close()


def plot_individual_model_shapley(model_name: str, model_label: str) -> None:
    """Generate per-model diagnostic plots."""
    results = load_shapley_results(model_name)
    if results is None:
        return
    ...
    bars = ax2.bar(
        range(len(metrics)),
        metrics["importance_median"],
        yerr=metrics["importance_iqr"],
        capsize=5,
        alpha=0.7,
        edgecolor="black",
    )
    ...
    for i, (bar, value) in enumerate(zip(bars, metrics["importance_median"])):
        ax2.text(
            bar.get_x() + bar.get_width() / 2,
            bar.get_height() + metrics["importance_iqr"].iloc[i],
            f"{value:.4f}",
            ha="center",
            va="bottom",
            fontweight="bold",
    ...
    # Direction
    ax3 = axes[1, 0]
    colors = ["green" if val > 0 else "red" for val in metrics["direction_median"]]
    bars = ax3.bar(
        range(len(metrics)),
        metrics["direction_median"],
        color=colors,
        alpha=0.7,
        edgecolor="black",
    )
    ...
    for _, (bar, value) in enumerate(zip(bars, metrics["direction_median"])):
        ax3.text(
            bar.get_x() + bar.get_width() / 2,
            value,
            f"{value:.4f}",
    ...
    bars = ax4.bar(
        range(len(metrics)),
        metrics["share_median"],
        yerr=metrics["share_iqr"],
        capsize=5,
        alpha=0.7,
        edgecolor="black",
    )
    ...
    for i, (bar, value) in enumerate(zip(bars, metrics["share_median"])):
        ax4.text(
            bar.get_x() + bar.get_width() / 2,
            bar.get_height() + metrics["share_iqr"].iloc[i],
            f"{value:.4f}",
            ha="center",
            va="bottom",
            fontweight="bold",
    ...
    plt.tight_layout()
    output_path = SHAPLEY_OUTPUT_DIR / f"shapley_{model_name}_detailed.png"
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
...
def main() -> None:
    """Generate all SHAP plots for BERT models."""
    print("=" * 80)
    print("Generating Shapley Value Plots for BERT Models")
    print("=" * 80)
